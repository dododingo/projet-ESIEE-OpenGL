...
public:
	...
	std::vector<int> types; //0 = point light, 1 = directional light, 2 = spot light.
	GLuint shaderprogram1;

	myLights(GLuint _shaderprogram1) {
		shaderprogram1 = _shaderprogram1;
	}

	void myLights::addLight(glm::vec4 position, glm::vec4 color, glm::vec3 direction, GLuint type)
	{
		positions.push_back(position);
		colors.push_back(color);
		directions.push_back(direction);
		types.push_back(type);
	}	void myLights::drawLight(int i)
	{
